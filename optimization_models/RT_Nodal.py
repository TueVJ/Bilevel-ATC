...
class RT_Nodal:
    ...
    def _build_variables(self):
        ...
        # Day-ahead production of generator g
        self.variables.gprod_DA = {}
        # Up- and Down-regulation of generator g at time t
        # Regulation is measured relative to the DA schedule.
        self.variables.gprod_up = {}
        self.variables.gprod_down = {}
        for t in taus:
            for g in generators:
                self.variables.gprod[g, t] = m.addVar(lb=0.0, ub=gendata[g]['capacity'])
    ...
    def _build_objective(self):
        ...
        m.setObjective(
            ...
            gb.quicksum(
                defaults.up_redispatch_premium*self.variables.gprod_up[gen, t] +
                defaults.down_redispatch_premium*self.variables.gprod_down[gen, t]
                for gen in self.data.generators for t in taus) +
    ...
    def _build_constraints(self):
        ...
        gprod_DA, gprod_up, gprod_down = self.variables.gprod_DA, self.variables.gprod_up, self.variables.gprod_down
        ...
        for t in taus:
            for n in nodes:
                injection_to_flow[n, t] = m.addConstr(
                    nodeinjection[n, t],
                    gb.GRB.EQUAL,
                    gb.quicksum(edgeflow[e, t] for e in outdict[n])
                    - gb.quicksum(edgeflow[e, t] for e in indict[n])
        ...
        for g in generators:
            for t in taus:
                schedulechange[g, t] = m.addConstr(
                    gprod[g, t],
                    gb.GRB.EQUAL,
                    gprod_DA[g, t] + gprod_up[g, t] - gprod_down[g, t])
        self.constraints.schedulechange = schedulechange
        # systembalance = {}
        # for t in taus:
    ...
    def _update_DA_schedule(self, othermodel):
        taus = self.data.taus
        generators = self.data.generators

        for g in generators:
            for t in taus:
                pda = othermodel.variables.gprod[g, t].x
                self.variables.gprod_DA[g, t].ub = pda
                self.variables.gprod_DA[g, t].lb = pda
